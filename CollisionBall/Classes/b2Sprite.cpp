...
b2Sprite::b2Sprite (HelloWorld * game, int type) {
    _game = game;
    _type = type;
}
...
void b2Sprite::update(float dt) {
    ...
    if (_body && isVisible()) {
        ...
        //only ball can rotate, player can not
        setRotation(this->getType() == kSpriteBall ? CC_RADIANS_TO_DEGREES(-1 * _body->GetAngle()):0);
    }
}
